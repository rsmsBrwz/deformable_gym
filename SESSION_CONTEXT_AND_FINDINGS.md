...
## 8. Ablationsstudien: 4 Iterationen im Überblick

Alle über `run_reward_ablation.py`, Ergebnisse jeweils gesichert (nicht überschrieben) unter `results/ablation_*`. Aktuellster Stand: `results/ablation/` (Iteration 4).
...
1. **DDPG/TD3-Policy-Kollaps als vorrangiges Problem (neu priorisiert nach Iteration 5):** Der einzige Lauf mit nicht-null `best_n_contacts_mean` (`phase1_warmstart`/DDPG) erwies sich bei genauer Prüfung als eingefrorene, vollständig gesättigte Policy (bit-identische Metriken über alle 20 Checkpoints, `action_saturation=1.0` durchgehend) – kein gelerntes Verhalten. Dieser Kollaps tritt reproduzierbar bereits beim allerersten Eval-Checkpoint (20.000 Schritte) auf. Nächster Schritt: Actor-Netzwerk-Init, `learning_starts`, Explorationsrauschen-Defaults von DDPG/TD3 gezielt untersuchen.
2. **PPO/SAC/A2C-Hyperparameter statt Trainingsdauer:** Diese drei zeigen keinen Kollaps, aber auch keinen sauberen Erfolg unter Warmstart – da 200k/400k/2M Schritte bereits durchprobiert wurden, liegt der Hebel vermutlich in Netzwerkgröße/Lernrate/Explorationsrauschen.
3. **~50–53%-Timeout-Quote als eigenständiges Problem, über 5 Iterationen unverändert:** Tritt gleichmäßig über alle Profile auf (auch mit Warmstart) – mit mehreren Seeds prüfen, ob das reproduzierbar an bestimmten Algorithmus/Profil-Kombinationen hängt oder zufällig verteilt ist. Bislang nur mit **1 Seed** gearbeitet (Zeitbudget-Entscheidung) – Konfidenz aller Ablationsbefunde entsprechend begrenzt.
...
5. `energy_quality` bleibt eine grobe Näherung (mischt Gravitations-/Elastizitätsenergie) – für eine sauberere Trennung wäre ein plugin-spezifischer Zugriff auf die Elastizitäts-Energie nötig (in MuJoCo aktuell nicht direkt exponiert).
6. Die Warmstart-Zielpose (Iteration 5) ist auf **eine** Box (`box_left`) kalibriert, nicht auf alle drei symmetrisch – für eine spätere Verallgemeinerung (z. B. zufällige Box-Auswahl pro Episode) müsste die Pose relativ zur tatsächlichen Objektposition berechnet werden statt als fixer Offset.
