...
Iterationen 1–5 über `run_reward_ablation.py` (Reward-Profile × Algorithmen), Ergebnisse gesichert unter `results/ablation_*`/`results/ablation/`. Iteration 6 über das neue `run_hparam_sweep.py` (SB3-Hyperparameter statt Reward-Profile variiert, s. [HYPERPARAMETER_SWEEP.md](HYPERPARAMETER_SWEEP.md)), Ergebnisse unter `results/hparam_sweep/`.
...
1. **`SAC/larger_net`-Folgelauf auswerten (läuft, Iteration 6b):** Multi-Seed (1–4, gleiches 400k-Budget) und ein verlängerter Einzellauf (Seed 0, 1,2 Mio. Schritte) sollen klären, ob der in Iteration 6 beobachtete wiederholte echte Kontakt (zwei unabhängige Checkpoints mit `n_contacts>0`, kein Kollaps) reproduzierbar ist bzw. sich mit mehr Zeit zu einem stabilen Griff festigt. Bislang bester, nicht-kollabierter Befund über alle 6 Iterationen – höchste Priorität für die nächste Auswertung.
2. **DDPG/TD3-Policy-Kollaps weiterhin offen:** Der einzige Iteration-5-Lauf mit nicht-null `best_n_contacts_mean` (`phase1_warmstart`/DDPG) erwies sich als eingefrorene, vollständig gesättigte Policy – kein gelerntes Verhalten. Tritt reproduzierbar bereits beim allerersten Eval-Checkpoint (20.000 Schritte) auf; in Iteration 6 zurückgestellt zugunsten des PPO/SAC/A2C-Sweeps. Nächster Schritt bei Gelegenheit: Actor-Netzwerk-Init, `learning_starts`, Explorationsrauschen-Defaults von DDPG/TD3 gezielt untersuchen.
