...
def register_mj_grasp_envs_shaped(
    version: int = 0,
):
    """Registers ShapedGraspEnv (dense reward shaping) under separate env
    ids, mirroring register_mj_grasp_envs() above. Additive only -- does not
    change the existing sparse-reward GraspEnv registrations.
    """
    robot_name2id = {
        "shadow_hand": "FloatingShadow",
        "mia_hand": "FloatingMia",
    ...
    for robot_name, robot_id in robot_name2id.items():
        for obj_name, obj_id in obj_name2id.items():
            ...
            kwargs.update(default_cam_config=cam_config)
            register(
                id=f"Mj{robot_id}Grasp{obj_id}Shaped-v{version}",
                entry_point="deformable_gym.envs.mujoco.shaped_grasp_env:ShapedGraspEnv",
                disable_env_checker=True,
                kwargs=kwargs,
            )
...
register_mj_grasp_envs_shaped(version=0)
