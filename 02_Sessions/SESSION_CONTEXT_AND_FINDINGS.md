...
## 8. Ablationsstudien: 8 Iterationen im Überblick

Iterationen 1–5 über `run_reward_ablation.py` (Reward-Profile × Algorithmen), Ergebnisse gesichert unter `results/ablation_*`/`results/ablation/`. Siehe [[../03_Experiments/REWARD_SHAPING_ABLATION]] für Iterationen 1–2. Iterationen 6–8 über `run_hparam_sweep.py`/`diagnose_action_noise.py` (SB3-Hyperparameter bzw. `action_noise` statt Reward-Profile variiert), Ergebnisse unter `results/hparam_sweep*/`/`results/diagnose_action_noise*/`. Siehe [[../03_Experiments/HYPERPARAMETER_SWEEP]] für Details zu Iterationen 6–8.
...
3. **DDPG-Policy-Kollaps – `action_noise` allein reicht nicht (Iteration 7, s. [[../03_Experiments/HYPERPARAMETER_SWEEP]]):** Anders als bei TD3 friert DDPGs Policy trotz `NormalActionNoise` weiterhin ein – nur an einem anderen (schlechteren) Fixpunkt als ohne Rauschen. Noch nicht untersucht: Actor-Netzwerk-Initialisierung, Critic-Lernrate, `learning_starts`, analog zur Kurzdiagnose-Methodik aus Iteration 7 (kurze 60k-Läufe vor einer vollen Ablation). Bislang zurückgestellt zugunsten der TD3-Spur (Punkt 2).
