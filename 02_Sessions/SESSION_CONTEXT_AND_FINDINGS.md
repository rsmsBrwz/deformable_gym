...
## 8. Ablationsstudien: 6 Iterationen im Überblick

Iterationen 1–5 über `run_reward_ablation.py` (Reward-Profile × Algorithmen), Ergebnisse gesichert unter `results/ablation_*`/`results/ablation/`. Siehe [[../03_Experiments/REWARD_SHAPING_ABLATION]] für Iterationen 1–2. Iteration 6 über das neue `run_hparam_sweep.py` (SB3-Hyperparameter statt Reward-Profile variiert), Ergebnisse unter `results/hparam_sweep/`. Siehe [[../03_Experiments/HYPERPARAMETER_SWEEP]] für Details zu Iteration 6.
...
1. **`SAC/larger_net`-Folgelauf auswerten (läuft, Iteration 6b, s. [[../03_Experiments/HYPERPARAMETER_SWEEP]]):** Multi-Seed (1–4, gleiches 400k-Budget) und ein verlängerter Einzellauf (Seed 0, 1,2 Mio. Schritte) sollen klären, ob der in Iteration 6 beobachtete wiederholte echte Kontakt (zwei unabhängige Checkpoints mit `n_contacts>0`, kein Kollaps) reproduzierbar ist bzw. sich mit mehr Zeit zu einem stabilen Griff festigt. Bislang bester, nicht-kollabierter Befund über alle 6 Iterationen – höchste Priorität für die nächste Auswertung.
2. **DDPG/TD3-Policy-Kollaps – Ursache teilweise identifiziert (Iteration 7, s. [[../03_Experiments/HYPERPARAMETER_SWEEP]]):** SB3 setzt für DDPG/TD3 standardmäßig `action_noise=None` (deterministische Policies ohne jedes eingebaute Explorationsrauschen) – keiner der bisherigen Läufe hat das konfiguriert. Ein gezielter Diagnoselauf (60k Schritte, `phase1_warmstart`, je mit/ohne `NormalActionNoise`) zeigt ein gemischtes Bild: Bei **TD3** verhindert das Rauschen das Einfrieren sichtbar (Reward/Aktion bleiben über alle 6 Checkpoints in Bewegung statt bit-identisch zu werden), findet in der kurzen Laufzeit aber noch keinen Kontakt. Bei **DDPG** friert die Policy trotz Rauschen weiterhin ein – nur an einem anderen (schlechteren) Fixpunkt als ohne Rauschen. **Hypothese also nur für TD3 bestätigt, für DDPG nicht.** Nächster Schritt: `action_noise` für TD3 in einem vollen Lauf (400k+) auf dem `phase1_warmstart`-Profil testen, ob sich daraus echter, gehaltener Kontakt entwickelt; für DDPG andere Ursachen (Actor-Init, Critic-Lernrate, `learning_starts`) weiter untersuchen.
3. **A2C zeigt jetzt ebenfalls ein Kollaps-Muster (neu in Iteration 6):** `A2C/high_lr` friert ab Schritt 300.000 bit-identisch ein (`action_saturation=1.0` bereits ab dem ersten Checkpoint) – bislang nur bei DDPG/TD3 beobachtet, jetzt auch bei einem on-policy-Algorithmus unter höherer Lernrate. Sollte bei der DDPG/TD3-Kollaps-Untersuchung (Punkt 2) mit betrachtet werden.
4. **~50–78%-Timeout-Quote als eigenständiges Problem, über 6 Iterationen nie unter 50%:** Iteration 6 zeigt mit 78 % die bislang höchste Quote. Tritt gleichmäßig über Profile/Varianten auf – mit mehreren Seeds prüfen, ob das reproduzierbar an bestimmten Algorithmus-Kombinationen hängt oder zufällig verteilt ist. Bislang nur mit **1 Seed** gearbeitet (Zeitbudget-Entscheidung, Ausnahme: der Iteration-6b-Folgelauf) – Konfidenz aller Ablationsbefunde entsprechend begrenzt.
5. **`retained_ratio > 0`-Gate (Iteration 5, s. [[../03_Experiments/REWARD_SHAPING_ABLATION]]) schließt die Phase-3-Sockelbonus-Lücke nur teilweise:** Sie kann bereits durch minimalen Kontakt beim Pause-Check am Episodenende feuern, auch wenn während der Episode selbst nie `grasped=True` gemessen wurde (beobachtet bei `phase1_2_3_warmstart`/DDPG: `retained_ratio_mean=0.333` bei `grasped_mean=0.0`). Für eine vollständige Schließung müsste die Gate zusätzlich einen Mindest-`grasped`/`n_contacts`-Wert *während* der Episode voraussetzen.
