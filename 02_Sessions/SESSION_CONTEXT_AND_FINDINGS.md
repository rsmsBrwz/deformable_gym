...
## 8. Ablationsstudien: 10 Iterationen im Überblick
...
| 10a (400k, DDPG `low_lr`, 5 Seeds) | `results/diagnose_ddpg_iter10a_low_lr/` | Alle 5 Seeds per `mj_step`-Hänger beendet (5/5 – schlechtere Quote als der sonstige ~50%-Schnitt). **Seed 3 zeigt den bislang stärksten Befund der gesamten Studie:** graduellen Reward-Anstieg über 5 Checkpoints (−0,40→0,98→0,49→1,32→7,02) bei kontinuierlich sinkender `action_saturation` (1,0→0,58), mündend in echten Kontakt (`n_contacts=13`) bei Schritt 140.000 – sieht nach echtem Lernfortschritt statt Zufallstreffer aus, aber Lauf hing sich direkt danach auf. Andere 4 Seeds: durchgehend `n_contacts=0`. |
| 10b (400k, PPO/SAC/A2C `low_gamma`/`combo_net_entropy`) | `results/hparam_sweep_iter10b/` | Kein neuer robuster Erfolg. `SAC/combo_net_entropy` reproduziert exakt das bereits in Iteration 6b widerlegte Muster (2 isolierte frühe Kontakt-Checkpoints, dann 18× `n_contacts=0`). `A2C/combo_net_entropy` zeigt den härtesten Kollaps der Studie (14 Checkpoints in Folge bit-identisch ab Schritt 140k). `SAC/low_gamma` bleibt nicht-eingefroren, aber ohne jeden Kontakt. |
...
3. ~~**DDPG `low_lr` auf vollem Budget + PPO/SAC/A2C breiter**~~ – **beide abgeschlossen (Iteration 10a/10b, s. [[../03_Experiments/HYPERPARAMETER_SWEEP]]):** 10b bringt nichts Neues (`SAC/combo_net_entropy` reproduziert nur das in 6b widerlegte Muster, `A2C/combo_net_entropy` zeigt den härtesten Kollaps der Studie). **10a liefert dagegen den bislang stärksten Befund über alle 10 Iterationen: `DDPG/low_lr`-Seed 3 zeigt einen graduellen, plausiblen Lernverlauf bis echtem Kontakt bei Schritt 140.000**, bevor der Lauf per `mj_step`-Hänger abbrach (alle 5 Seeds hingen sich auf, schlechtere Quote als sonst üblich). **Nächster Schritt (Iteration 11):** Seed 3 mit identischem Setup wiederholen, um Reproduzierbarkeit und Hänger-Verhalten zu prüfen, statt weiterer neuer Hyperparameter-Achsen.
